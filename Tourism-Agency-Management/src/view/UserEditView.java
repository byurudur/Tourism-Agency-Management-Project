...
public class UserEditView extends Layout {
    ...
    public UserEditView(User user) {
        ...
        btn_save.addActionListener(new ActionListener() { // Section - 7 : The admin user type has been coded so that it can perform operations such as employee listing, adding, updating and deleting mentioned in the project.
            @Override
            public void actionPerformed(ActionEvent e) {
                ...
                if (Helper.isFieldEmpty(fld_user_name) && Helper.isFieldEmpty(fld_password) && Helper.isFieldEmpty((JTextField) cmb_perm.getSelectedItem())) {
                    ...
                    if (user.getUser_id() != 0) {
                        ...
                        if(!userManager.update(user)){
                            ...
                            Helper.getMessage("Update a User", "Information");// Section 24-25 : The user is given appropriate pop up messages for successful transactions. Appropriate error messages are given to the user for incorrect operations.
